...
from src.tools.dataset_5folds import create_dataset_for_5folds

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger()

parser = argparse.ArgumentParser(description='Train a model for drug-target interaction prediction using 5-fold cross validation')
parser.add_argument('dataset_index', type=int, choices=[0, 1], help='Index for dataset: 0 for davis, 1 for kiba')
parser.add_argument('fold_index', type=int, choices=[0, 1, 2, 3, 4], help='Fold index for cross-validation (0-4)')
...
parser.add_argument('--save_path', type=str, default='weights/fold/', help='Path to save model weights')
...
for dataset in datasets:
    train_data, valid_data = create_dataset_for_5folds(dataset, fold, model.__class__.__name__)
    ...
    best_mse = float('inf')
    best_epoch = -1
    model_file_name = f'{args.save_path}/{dataset}/v{version}_fold_{fold}_{model.__class__.__name__}_{dataset}.model'

    for epoch in range(args.num_epochs):
        epoch_start_time = time.time()
        train(model, device, train_loader, optimizer, epoch + 1)
        logger.info('Predicting for validation data')
        ...
        if val < best_mse:
            ...
            logger.info(f'MSE improved at epoch {best_epoch}; best_validation_mse: {best_mse}, {dataset}, fold: {fold}')
            no_improve_count = 0
        else:
            no_improve_count += 1
            logger.info(f'No improvement since epoch {best_epoch}; best_validation_mse: {best_mse}, {dataset}, fold: {fold}')

        # Save the model periodically
        if (epoch + 1) % args.save_interval == 0:
            checkpoint_file_name = f'{args.save_path}/{dataset}/v{version}_fold_{fold}_{model.__class__.__name__}_{dataset}_epoch_{epoch + 1}.model'

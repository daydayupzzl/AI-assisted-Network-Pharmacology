...
from src.tools.utils import collate
from src.models.GraphkmerDTA.GraphkmerDTA import GraphkmerDTA as GNNNet

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger()

parser = argparse.ArgumentParser(description='Test a trained model for drug-target interaction prediction using 5-fold cross-validation')
...
model_file_name = f'weights/fold/{dataset}/v{version}_fold_{fold}_{model_st}_{dataset}.model'
result_file_name = f'results/fold/{dataset}/v{version}_fold_{fold}_test_on_{dataset}.txt'
model = GNNNet().to(device)
model.load_state_dict(torch.load(model_file_name, map_location=cuda_name), strict=False)

_, test_data = create_dataset_for_5folds(dataset, fold, model_st)
